...
unique_ptr<ExpressionState> ExpressionExecutor::InitializeState(const Expression &expr,
	...
	//	Add ExpressionClass ONNX
	case ExpressionClass::BOUND_ONNX:
		return InitializeState(expr.Cast<BoundOnnxExpression>(), state);
	default:
...
void ExpressionExecutor::Execute(const Expression &expr, ExpressionState *state, const SelectionVector *sel,
	...
	// Todo
	case ExpressionClass::BOUND_ONNX:
		Execute(expr.Cast<BoundOnnxExpression>(), state, sel, count, result);
		break;		
	default:
